{*
  Importe l'unité FunLabyCoreConsts dans un environnement Sepi
  @author sjrd
  @version 1.0
*}
unit SepiImportsFunLabyCoreConsts;
...
var
  SepiImportsFunLabyCoreConstsLazyLoad: Boolean = False;
...
const // don't localize
  UnitName = 'FunLabyCoreConsts';
  ResourceName = 'SepiImportsFunLabyCoreConsts';
  TypeCount = 1;
  MethodCount = 1;
  VariableCount = 1;
...
begin
  ...
  try
    Result := TSepiUnit.LoadFromStream(Root, Stream,
      SepiImportsFunLabyCoreConstsLazyLoad,
      TGetMethodCodeEvent(GetMethodCodeEvent),
      TGetTypeInfoEvent(GetTypeInfoEvent),
      TGetVarAddressEvent(GetVarAddressEvent));

    if SepiImportsFunLabyCoreConstsLazyLoad then
...
begin
  ...
  WriteLn(ErrOutput, Format('InstanceSize;%d;%d;FunLabyCoreConsts;%s;%s',
...
initialization
  ...
  SepiRegisterImportedUnit('FunLabyCoreConsts', ImportUnit);
finalization
  SepiUnregisterImportedUnit('FunLabyCoreConsts');
end.
