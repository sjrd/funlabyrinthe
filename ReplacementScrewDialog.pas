...
uses
  ScrewNumberDialog, EditLabyrintheMain;
...
begin
  ...
  if Result = 3 then
  begin
    Result := FormNumeroCase.GetTeleporterNumber+96;
  end else if Result = 4 then
  begin
    Result := FormNumeroCase.GetButtonNumber;
    if (Result <= 15) then inc (Result, 32) else inc (Result, 145);
  end else if Result = 5 then
  begin
    Result := FormNumeroCase.GetBoatNumber+192;
  end;
end;
